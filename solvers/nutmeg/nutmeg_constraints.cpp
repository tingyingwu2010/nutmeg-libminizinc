...
#define ARRAY(X) eval_array_lit(s.env().envi(), call->arg(X))
     ...
     void p_int_eq(SolverInstanceBase& s, const Call* call) {
         SOL.add_constr_linear({INTVAR(0), INTVAR(1)}, {1, -1}, Nutmeg::Sign::EQ, 0);
     }
    ...
    // }
     ...
     void p_int_le(SolverInstanceBase& s, const Call* call) {
         SOL.add_constr_linear({INTVAR(0), INTVAR(1)}, {1, -1}, Nutmeg::Sign::LE, 0);
     }

     void p_int_lt(SolverInstanceBase& s, const Call* call) {
         SOL.add_constr_linear({INTVAR(0), INTVAR(1)}, {1, -1}, Nutmeg::Sign::LE, -1);
     }
    ...
    // }
     ...
     void p_int_lin_eq(SolverInstanceBase& s, const Call* call) {
         SOL.add_constr_linear(INTVARARRAY(1), INTARRAY(0), Nutmeg::Sign::EQ, INT(2));
     }

     void p_int_lin_ne(SolverInstanceBase& s, const Call* call) {
         SOL.add_constr_linear_neq(INTVARARRAY(1), INTARRAY(0), INT(2));
     }

     void p_int_lin_le(SolverInstanceBase& s, const Call* call) {
         SOL.add_constr_linear(INTVARARRAY(1), INTARRAY(0), Nutmeg::Sign::LE, INT(2));
     }
    ...
    // }
     ...
     void p_array_int_element(SolverInstanceBase& s, const Call* call) {
       assert(ARRAY(1)->min(0) == 1 && ARRAY(1)->max(0) == ARRAY(1)->size());
//         Vector<Int> vals = INTARRAY(1);
         if (PAR(0)) {
             not_yet_implemented();
...
//             }
             ...
             SOL.add_constr_element(INTVAR(0), INTARRAY(1), INTVAR(2));
         }
     }
...
//         }
           ...
           SOL.add_constr_element(INTVAR(0), INTVARARRAY(1), INTVAR(2));
       }
     }
    ...
    // }
     ...
     void p_all_different(SolverInstanceBase& s, const Call* call) {
         SOL.add_constr_alldifferent(INTVARARRAY(0));
     }
